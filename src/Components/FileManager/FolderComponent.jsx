...
const FolderComponent = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${backendUrl}/api/folders/${folderId}/folders`)
      .then(response => setFolderContent(response.data))
      .catch(error => {
        if (error.response) {
          if (error.response.status === 404) {
  ...
  const deleteFileFromFolder = async (fileId) => {
    try {
      ...
      const response = await axios.get(`${backendUrl}/api/folders/${folderId}/files`);
      setFileContent(response.data);
    } catch (error) {
      console.error('Error deleting file:', error);
    }
  ...
  const handleFileUpload = async () => {
    try {
      ...
      await axios.post(`${backendUrl}api/folders/${folderId}/upload`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  const handleCreateFolder = async () => {
    try {
      ...
      const response = await axios.post(`${backendUrl}/api/folders/${folderId}/create-folder`, { name: newFolderName });
  ...
  const handleDownloadFile = (file) => {
    ...
    window.location.href = `${backendUrl}/api/folders/${folderId}/files/${file._id}/download`;
  };
